...
describe('Error classes exported from @torquedev/core index', () => {
  ...
  it('total exports are 15 (6 existing + 5 errors + 4 IDD primitives)', async () => {
    const mod = await import('../index.js');
    const keys = Object.keys(mod);
    ...
    // 4 IDD primitives: Intent, Behavior, Context, AgentRouter
    assert.equal(keys.length, 15, `Expected 15 exports, got ${keys.length}: ${keys.join(', ')}`);
  });
});
